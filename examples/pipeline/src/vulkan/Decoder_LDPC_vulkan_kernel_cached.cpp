...
to launch() with the vector it already owns. The per-frame host path is then just
the key-view build inside launch() plus the stream's cache scan.
	...
	spu::device_interface::GpuStream* stream)
{
	...
	// send_count) read it from the CPU; launch() already fence-waited, so the
