...
int main(int argc, char** argv)
{
    ...
    spu::module::Switcher swi(2, N_, typeid(float));

    spu::module::Iterator cnt(I);
    spu::module::Initializer<float> zeros(N_);
    ...
    enc_n  [             "encode::U_K"     ] = src    [           "generate::out_data" ];
    itl_bit[         "interleave::nat"     ] = enc_n  [             "encode::X_N"      ];
    enc_i  [             "encode::U_K"     ] = itl_bit[         "interleave::itl"      ];
    mdm    [           "modulate::X_N1"    ] = enc_i  [             "encode::X_N"      ];
    chn    [          "add_noise::X_N "    ] = mdm    [           "modulate::X_N2"     ];
    mdm    [         "demodulate::Y_N1"    ] = chn    [          "add_noise::Y_N"      ];
    zeros  (         "initialize"          ) = mdm    [         "demodulate::Y_N2"     ];
    swi    [             "select::in_data1"] = zeros  [         "initialize::out"      ];
    ext    ["add_sys_and_ext_llr::ext"     ] = swi    [         "select::out_data"     ];
    ext    ["add_sys_and_ext_llr::Y_N1"    ] = mdm    [         "demodulate::Y_N2"     ];
    dec_i  [        "decode_siso::Y_N1"    ] = ext    ["add_sys_and_ext_llr::Y_N2"     ];
    ext    [        "get_sys_llr::Y_N"     ] = dec_i  [        "decode_siso::Y_N2"     ];
    itl_llr[       "deinterleave::itl"     ] = ext    [        "get_sys_llr::Y_K "     ];
    swi    [            "commute::in_ctrl" ] = cnt    [            "iterate::out"      ];
    swi    [            "commute::in_data" ] = itl_llr[       "deinterleave::nat"      ];
    dec_n  [        "decode_siso::Y_N1"    ] = swi    [            "commute::out_data0"];
    itl_llr[         "interleave::nat"     ] = dec_n  [        "decode_siso::Y_N2"     ];
    swi    [             "select::in_data0"] = itl_llr[         "interleave::itl"      ];
    dec_n  [        "decode_siho::Y_N"     ] = swi    [            "commute::out_data1"];
    mnt    [       "check_errors::U"       ] = src    [           "generate::out_data" ];
    mnt    [       "check_errors::V"       ] = dec_n  [        "decode_siho::V_K"      ];
    cnt    (            "iterate"          ) = ext    ["add_sys_and_ext_llr::status"   ];
    chn    [          "add_noise::CP"      ] = sigma;
    mdm    [         "demodulate::CP"      ] = sigma;

    spu::runtime::Sequence sequence(src("generate"), nthreads);
    ...
    for (auto& mod : sequence.get_modules<spu::module::Module>(false))
    ...
    std::mt19937 prng;
    for (auto &m : sequence.get_modules<spu::tools::Interface_set_seed>())
        m->set_seed(prng());
    ...
    for (auto ebn0 = ebn0_min; ebn0 < ebn0_max; ebn0 += ebn0_step)
    {
        ...
        sequence.exec([&monitor_red]() { return monitor_red.is_done(); });
    ...
    std::cout << "#" << std::endl;
    spu::tools::Stats::show(sequence.get_modules_per_types(), true);
